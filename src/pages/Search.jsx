        ...
        <div className="max-w-7xl mx-auto px-4 py-8 min-h-[80vh]">
            ...
            <div className="flex flex-col gap-6 mb-12">
                <div className="relative group">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl py-6 pl-16 pr-6 text-xl text-white placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-2xl"
                        value={query}
                    ...
                    {query && (
                        <button
                            onClick={() => setQuery('')}
                            className="absolute right-6 top-1/2 -translate-y-1/2 p-2 bg-white/5 hover:bg-white/10 rounded-full text-slate-400 transition-all"
                        >
                            <X size={18} />
                        </button>
                    )}
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            showFilters ? "bg-white/10 text-white border-white/20" : "bg-white/5 text-slate-400 border-white/5 hover:bg-white/10"
                        )}
                    >
                        <Filter size={16} />
                        Advanced Filters
                ...
                <AnimatePresence>
                    {showFilters && (
                        ...
                        >
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 bg-slate-900/50 rounded-2xl border border-white/10">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-slate-800 border border-white/5 rounded-xl p-3 text-white outline-none focus:border-brand-500 transition-all font-medium"
                                    />
                                </div>
                                <div>
            ...
            {totalResults > 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.tournaments.map(t => (
                                    <motion.div variants={item} key={t.id}>
                                        <Link to={`/tournaments/${t.id}`} className="block group p-4 bg-slate-900 border border-white/5 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-brand-500/10 rounded-xl flex items-center justify-center text-brand-400 group-hover:bg-brand-500 group-hover:text-white transition-all">
                                                    <Trophy size={20} />
                                                </div>
                                                <div>
                    ...
                    {results.teams.length > 0 && (
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.teams.map(t => (
                                    <motion.div variants={item} key={t.id}>
                                        <Link to={`/teams`} className="block group p-4 bg-slate-900 border border-white/5 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center overflow-hidden border border-white/5">
                                                    {t.logoUrl ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.players.map(p => (
                                    <motion.div variants={item} key={p.id}>
                                        <Link to={`/players`} className="block group p-4 bg-slate-900 border border-white/5 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center overflow-hidden border border-white/5">
                                                    {p.photoUrl ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {results.matches.map(m => (
                                    <motion.div variants={item} key={m.id}>
                                        <Link to={`/live/${m.id}`} className="block group p-6 bg-slate-900 border border-white/5 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex justify-between items-center mb-4">
                                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{m.competition}</span>
                                                <span className={cn(
                                                    "px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border",
                                                    m.status === 'live' ? "bg-red-500/10 text-red-500 border-red-500/20 animate-pulse" : "bg-white/5 text-slate-500 border-white/5"
                                                )}>{m.status}</span>
                                            </div>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    <div className="w-20 h-20 bg-white/5 rounded-3xl flex items-center justify-center text-slate-500 mb-6 border border-white/5">
                        <SearchIcon size={40} />
                    </div>
