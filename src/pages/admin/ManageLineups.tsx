...
const ManageLineups = () => {
    ...
    // â”€â”€â”€ STANDARD (PLAYER-ID) MODE HANDLERS â”€â”€â”€
        ...
        try {
            if (lineup.id) {
                ...
                logAuditEvent('UPDATE_LINEUP', {
                    entityType: 'lineup',
                    entityId: lineup.id,
                    entityName: `${selectedTeam} â€” ${matchLabel}`,
                });
        ...
        try {
            ...
            if (manualLineup.id) {
                ...
                logAuditEvent('UPDATE_LINEUP', {
                    ...
                    entityName: `${selectedTeam} â€” ${matchLabel} (Manual)`,
                });
    ...
    return (
        <div className="container mx-auto px-4 py-8 text-white">
                ...
                >
                    ...
                    {scopedMatches.filter(m => m.status !== 'finished').map(match => (
                        <option key={match.id} value={match.id}>
                            {match.isQuickMatch ? 'âš¡ ' : ''}{match.competition} - {match.teamA} vs {match.teamB}
                ...
                <div className="mb-6">
                    ...
                    {isManualMode && (
                        <div className="mt-2 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-600 dark:text-amber-400 text-xs font-bold flex items-center gap-2">
                            <UserPlus size={14} />
                            Manual Mode: Type player names directly â€” perfect for quick matches and unregistered teams.
                        </div>
                    )}
                </div>
            )}
            ...
            {/* â•â•â• STANDARD MODE (FROM ROSTER) â•â•â• */}
            {selectedTeam && !isManualMode && (
            ...
            {!selectedMatch && (
                <div className="bg-[#0B1220] dark:bg-gray-800 p-6 rounded-lg mt-6">
                    ...
                    <ol className="list-decimal list-inside space-y-2 text-[#94A3B8] dark:text-gray-300">
                        ...
                        <li>Add players to the Starting lineup (any number â€” even 3 or 5 players work!)</li>
                    ...
                    <div className="mt-4 p-3 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-600 dark:text-amber-400 text-sm font-medium">
                        ðŸ’¡ <strong>Tip:</strong> For Quick Matches with unregistered teams, Manual Entry mode is auto-enabled so you can type player names directly!
                    </div>
                </div>
            )}
        </div>
