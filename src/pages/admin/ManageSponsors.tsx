...
const ManageSponsors = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            <h1 className="text-3xl font-display font-bold text-white mb-6">Manage Sponsors</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1">
                    <div className="glass-card p-6 rounded-2xl sticky top-24">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-[#94A3B8] mb-1">Company/Sponsor Name*</label>
                                <input
                                    type="text"
                                    required
                                    className="w-full bg-black/20 border border-[#24344D] rounded-lg px-4 py-2 text-white focus:ring-1 focus:ring-brand-500 outline-none"
                                    value={formData.name}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-[#94A3B8] mb-1">Website URL (Optional)</label>
                                <input
                                    type="text"
                                    className="w-full bg-black/20 border border-[#24344D] rounded-lg px-4 py-2 text-white focus:ring-1 focus:ring-brand-500 outline-none"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-[#94A3B8] mb-1">Tier</label>
                                    <select
                                        className="w-full bg-black/20 border border-[#24344D] rounded-lg px-4 py-2 text-white focus:ring-1 focus:ring-brand-500 outline-none"
                                        value={formData.tier}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-[#94A3B8] mb-1">Logo Image*</label>
                                <input
                                    type="file"
                                    ref={fileInputRef}
                                ...
                                <div className="space-y-3">
                                    <input
                                        ...
                                        className="w-full bg-black/20 border border-[#24344D] rounded-lg px-4 py-2 text-white focus:ring-1 focus:ring-brand-500 outline-none text-sm"
                                    ...
                                    <div className="flex gap-2 w-full">
                                        <button
                                            ...
                                            disabled={uploading}
                                            className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-[#131D31]/50 hover:bg-[#131D31]/10 border border-[#24344D] rounded-lg text-sm text-[#94A3B8] transition-colors w-full"
                                        >
                                            <Upload size={16} />
                                    ...
                                    {/* Preview */}
                                    {formData.logoUrl && (
                                        <div className="relative mt-2 p-4 rounded-lg bg-[#1E2B42] border border-[#24344D] flex items-center justify-center group h-32">
                            ...
                            <div className="flex items-center gap-2 mt-4 pt-2 border-t border-[#24344D]">
                                <input
                                    type="checkbox"
                                    id="activeStatus"
                                    checked={formData.active}
                                    onChange={e => setFormData({ ...formData, active: e.target.checked })}
                                    className="w-4 h-4 text-brand-400 bg-black/20 border-[#24344D] rounded rounded-lg focus:ring-brand-500 focus:ring-2"
                                />
                                <label htmlFor="activeStatus" className="text-sm font-medium text-[#94A3B8]">
                            ...
                            <button
                                type="submit"
                                disabled={uploading}
                                className="w-full bg-brand-600 hover:bg-brand-500 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-brand-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 mt-4"
                            >
                                {uploading ? (
                                    <>
                                        <div className="h-4 w-4 border-2 border-[#24344D]/30 dark:border-white/30 border-t-white rounded-full animate-spin" />
                ...
                <div className="lg:col-span-2 space-y-4">
                    <h2 className="text-xl font-bold text-white mb-4">Current Sponsors</h2>
                    {loading ? (
                        <div className="text-center py-10 text-[#94A3B8]">Loading...</div>
                    ) : (
                        <div className="space-y-4">
                            {sponsors.map(item => (
                                ...
                                >
                                    <div className="w-24 h-16 bg-[#131D31]/50 rounded-lg overflow-hidden flex-shrink-0 flex items-center justify-center p-2">
                                        {item.logoUrl ? (
                                            <img src={item.logoUrl} alt={item.name} className="max-w-full max-h-full object-contain" />
                                        ) : (
                                            <Image size={24} className="text-[#94A3B8]" />
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start">
                                            <div>
                                                ...
                                                <h3 className="font-bold text-white text-lg leading-tight truncate flex items-center gap-2">
                                                    {item.name}
                                                ...
                                                {(isSuperAdmin || item.uploadedBy === currentUser?.uid) && (
                                                    <>
                                                        <button
                                                            ...
                                                            className={`p-2 transition-colors ${item.active ? 'text-green-500 hover:text-green-400' : 'text-[#64748B] hover:text-green-500'}`}
                                                        ...
                                                        <button
                                                            ...
                                                            className="text-[#64748B] hover:text-brand-400 p-2 transition-colors"
                                                        >
                                                            <Edit2 size={18} />
                                                        </button>
                                                        {isSuperAdmin && (
                                                            <button
                                                                ...
                                                                className="text-[#64748B] hover:text-red-500 p-2 transition-colors"
                                                            >
                                                                <Trash2 size={18} />
                                                            </button>
                                                        )}
                                        ...
                                        {item.websiteUrl && (
                                            <a href={item.websiteUrl} target="_blank" rel="noopener noreferrer" className="text-[#64748B] hover:text-brand-400 text-xs mt-1 flex items-center gap-1 truncate max-w-xs">
                            ...
                            {sponsors.length === 0 && (
                                <div className="text-center py-10 bg-[#131D31]/50 rounded-xl border border-dashed border-[#24344D] text-[#64748B]">
