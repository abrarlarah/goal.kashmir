        ...
        <div className="max-w-7xl mx-auto px-4 py-8 min-h-[80vh]">
            ...
            <div className="flex flex-col gap-6 mb-12">
                <div className="relative group">
                    ...
                    <input
                        ...
                        className="w-full bg-[#131D31]/60 dark:bg-[#0B1220]/60 backdrop-blur-xl border border-[#24344D] rounded-2xl py-6 pl-16 pr-6 text-xl text-white placeholder:text-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all shadow-2xl"
                        value={query}
                    ...
                    {query && (
                        <button
                            onClick={() => setQuery('')}
                            className="absolute right-6 top-1/2 -translate-y-1/2 p-2 bg-[#131D31]/50 hover:bg-[#131D31]/10 rounded-full text-[#94A3B8] transition-all"
                        >
                            <X size={18} />
                        </button>
                    )}
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeFilter === f.id
                                        ...
                                        : "bg-[#131D31]/50 text-[#94A3B8] border-[#24344D]/40 hover:bg-[#131D31]/10"
                                )}
                            >
                                <f.icon size={16} />
                                {f.label}
                    ...
                    <button
                        ...
                        className={cn(
                            ...
                            showFilters ? "bg-[#1E2B42] text-white border-[#24344D]" : "bg-[#131D31]/50 text-[#94A3B8] border-[#24344D]/40 hover:bg-[#131D31]/10"
                        )}
                    >
                        <Filter size={16} />
                        Advanced Filters
                ...
                <AnimatePresence>
                    {showFilters && (
                        ...
                        >
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6 bg-[#131D31]/60 dark:bg-[#0B1220]/60 rounded-2xl border border-[#24344D]">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-[#0B1220] bg-[#131D31] border border-[#24344D]/40 rounded-xl p-3 text-white outline-none focus:border-brand-500 transition-all font-medium"
                                    />
                                </div>
                                <div>
            ...
            {totalResults > 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.tournaments.map(t => (
                                    <motion.div variants={item} key={t.id}>
                                        <Link to={`/tournaments/${t.id}`} className="block group p-4 bg-[#0B1220] border border-[#24344D]/40 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-white group-hover:text-brand-400 transition-colors">{t.name}</h3>
                                                    <p className="text-xs text-[#64748B]">{t.district} â€¢ {t.type}</p>
                                                </div>
                    ...
                    {results.teams.length > 0 && (
                        <section>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.teams.map(t => (
                                    <motion.div variants={item} key={t.id}>
                                        <Link to={`/teams`} className="block group p-4 bg-[#0B1220] border border-[#24344D]/40 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-[#0B1220] bg-[#131D31] rounded-xl flex items-center justify-center overflow-hidden border border-[#24344D]/40">
                                                    {t.logoUrl ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {results.players.map(p => (
                                    <motion.div variants={item} key={p.id}>
                                        <Link to={`/players`} className="block group p-4 bg-[#0B1220] border border-[#24344D]/40 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-[#0B1220] bg-[#131D31] rounded-xl flex items-center justify-center overflow-hidden border border-[#24344D]/40">
                                                    {p.photoUrl ? (
                                                ...
                                                <div>
                                                    <h3 className="font-bold text-white group-hover:text-brand-400 transition-colors">{p.name}</h3>
                                                    <p className="text-xs text-[#64748B]">{p.team} â€¢ {p.position}</p>
                                                </div>
                                                <ChevronRight className="ml-auto text-[#94A3B8] group-hover:text-brand-500 transition-colors" size={18} />
                                            </div>
                                        </Link>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {results.matches.map(m => (
                                    <motion.div variants={item} key={m.id}>
                                        <Link to={`/live/${m.id}`} className="block group p-6 bg-[#0B1220] border border-[#24344D]/40 rounded-2xl hover:border-brand-500/30 transition-all shadow-lg">
                                            <div className="flex justify-between items-center mb-4">
                                                <span className="text-[10px] font-black text-[#64748B] uppercase tracking-widest">{m.competition}</span>
                                                <span className={cn(
                                                    "px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border",
                                                    m.status === 'live' ? "bg-red-500/10 text-red-500 border-red-500/20 animate-pulse" : "bg-[#131D31]/50 text-[#64748B] border-[#24344D]/40"
                                                )}>{m.status}</span>
                                            </div>
                                            <div className="flex items-center justify-between gap-4">
                                                <div className="flex-1 flex flex-col items-center">
                                                    <span className="font-bold text-white group-hover:text-brand-400 transition-colors text-center truncate w-full">{m.teamA}</span>
                                                </div>
                                                <div className="px-4 py-2 bg-black/40 rounded-xl border border-[#24344D]/40">
                                                    <span className="font-black text-xl text-white">
                                                        {m.status === 'scheduled' ? m.time : `${m.scoreA} - ${m.scoreB}`}
                                                    </span>
                                                </div>
                                                <div className="flex-1 flex flex-col items-center">
                                                    <span className="font-bold text-white group-hover:text-brand-400 transition-colors text-center truncate w-full">{m.teamB}</span>
                                                </div>
                                            </div>
                                            <div className="mt-4 pt-4 border-t border-[#24344D]/40 flex justify-between items-center">
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    <div className="w-20 h-20 bg-[#131D31]/50 rounded-3xl flex items-center justify-center text-[#64748B] mb-6 border border-[#24344D]/40">
                        <SearchIcon size={40} />
                    </div>
                    <h3 className="text-2xl font-black text-white italic mb-2">No results found for "{query}"</h3>
