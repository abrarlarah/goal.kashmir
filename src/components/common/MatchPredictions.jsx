...
const MatchPredictions = ({ matchId, teamA, teamB }) => {
    ...
    if (loading && totalVotes === 0) {
        return (
            <div className="bg-slate-900/50 dark:bg-white/5 backdrop-blur-sm rounded-3xl p-6 border border-white/5 animate-pulse h-64" />
        );
    }

    return (
        <div className="bg-slate-900/50 dark:bg-white/5 backdrop-blur-sm rounded-3xl p-6 border border-white/5 shadow-xl">
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5">
                    ...
                    <span className="text-xs font-black text-slate-400">{totalVotes} Votes</span>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {choices.map(choice => (
                            <button
                                ...
                                className="group relative overflow-hidden bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl p-4 transition-all hover:border-brand-500/30 font-bold"
                            >
                                <span className="relative z-10 text-xs font-black text-slate-400 group-hover:text-white uppercase transition-colors">
                                    {choice.label}
                                </span>
                            </button>
                        ))}
