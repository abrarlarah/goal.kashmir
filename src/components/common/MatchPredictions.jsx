...
const MatchPredictions = ({ matchId, teamA, teamB }) => {
    ...
    if (loading && totalVotes === 0) {
        return (
            <div className="bg-white/50 dark:bg-slate-900/50 bg-[#131D31]/50 backdrop-blur-sm rounded-3xl p-6 border border-[#24344D]/30 border-[#24344D]/50 animate-pulse h-64" />
        );
    }

    return (
        <div className="bg-white/50 dark:bg-slate-900/50 bg-[#131D31]/50 backdrop-blur-sm rounded-3xl p-6 border border-[#24344D]/30 border-[#24344D]/50 shadow-xl">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-display font-black text-white text-white italic uppercase tracking-wider text-sm">Win Probability</h3>
                        <p className="text-[10px] text-[#64748B] font-bold uppercase">Fan Predictions</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 px-3 py-1 bg-[#131D31]/50 rounded-full border border-[#24344D]/30 border-[#24344D]/50">
                    <Users size={12} className="text-[#64748B]" />
                    <span className="text-xs font-black text-[#94A3B8] text-[#94A3B8]">{totalVotes} Votes</span>
                </div>
            </div>
            ...
            <div className="space-y-6">
                ...
                {!userVote ? (
                    <div className="grid grid-cols-3 gap-3">
                        {choices.map(choice => (
                            <button
                                ...
                                className="group relative overflow-hidden bg-[#131D31]/50 hover:bg-white/10 border border-[#24344D]/30 border-[#24344D]/50 rounded-2xl p-4 transition-all hover:border-brand-500/30 font-bold"
                            >
                                <span className="relative z-10 text-xs font-black text-[#94A3B8] text-[#94A3B8] group-hover:text-slate-900 text-white uppercase transition-colors">
                                    {choice.label}
                                </span>
                            </button>
                        ))}
                    ...
                    <div className="space-y-6">
                                        ...
                                        <div className="flex justify-between items-end mb-2">
                                            <div className="flex items-center gap-2">
                                                <span className={cn(
                                                    ...
                                                    isSelected ? "text-brand-400" : "text-[#94A3B8] text-[#94A3B8]"
                                                )}>
                                                    {choice.label}
                                                </span>
                                                {isSelected && <CheckCircle2 size={14} className="text-brand-400" />}
                                            </div>
                                            <span className="text-lg font-impact text-white text-white">{Math.round(percent)}%</span>
                                        </div>
                                        <div className="h-3 w-full bg-[#101827] bg-[#131D31]/50 rounded-full overflow-hidden border border-[#24344D]/30 border-[#24344D]/50 p-[1px]">
                                            <motion.div
                                                initial={{ width: 0 }}
                        ...
                        <div className="pt-4 border-t border-[#24344D]/30 border-[#24344D]/50">
                                    ...
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            userVote === choice.id
                                                ...
                                                : "bg-[#131D31]/50 text-[#64748B] border-[#24344D]/30 border-[#24344D]/50 hover:bg-white/10 hover:text-white text-white"
                                        )}
                                    >
                                        {choice.label}
                                    </button>
