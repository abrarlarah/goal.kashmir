...
const SubstitutionManager = ({ lineup, players, onSubstitutionComplete }) => {
    ...
    const handleSubstitution = async () => {
        ...
        try {
            ...
            alert(`Substitution: ${playerOut?.name} âž¡ï¸ ${playerIn?.name}`);
        } catch (error) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select
                        ...
                        className="w-full bg-[#101827] dark:bg-gray-700 text-white p-2 rounded border border-gray-600"
                    >
                ...
                <div className="bg-[#101827] dark:bg-gray-700 p-3 rounded mb-4 text-sm">
                    <div className="flex items-center justify-center gap-4">
                        <span className="text-red-400">
                            â¬‡ï¸ {getPlayer(selectedStarter)?.name}
                        </span>
                        <span className="text-[#64748B] dark:text-gray-400">â†’</span>
                        <span className="text-green-400">
                            â¬†ï¸ {getPlayer(selectedBench)?.name}
                        </span>
                    </div>
                </div>
            )}
